 ...
 */
    ...
    (() => {
                ...
                function setSpollerAction(e) {
                    ...
                    if (el.closest("[data-spoller]")) {
                        ...
                        if (!spollersBlock.querySelectorAll("._slide").length) {
                            ...
                            spollerTitle.parentElement.classList.toggle("_spoller-open");
                            _slideToggle(spollerTitle.nextElementSibling, spollerSpeed);
                        }
                        if (document.querySelectorAll(".filter-catalog-item__button._spoller-active").length) document.documentElement.classList.add("filter-open"); else document.documentElement.classList.remove("filter-open");
                        e.preventDefault();
                    }
                }
        ...
        const inputMasks = document.querySelectorAll("[data-input-number]");
        ...
        function initSliders() {
            ...
            if (document.querySelector(".product-card__images")) {
                const thumbsSwiper = new swiper_core_Swiper(".thumb-slider", {
                    modules: [ Navigation, freeMode, Thumb ],
                    watchOverflow: true,
                    observer: true,
                    observeParents: true,
                    slidesPerView: 3,
                ...
                new swiper_core_Swiper(".slider-product-card", {
                    modules: [ Navigation, freeMode, Thumb ],
                    watchOverflow: true,
                    observer: true,
                    observeParents: true,
...
***************************************************************************** */
        ...
        window.onload = function() {
            const documentActions = e => {
                const targetElement = e.target;
                if (targetElement.closest(".search-header__button-js")) {
                    document.querySelector(".search-form").classList.toggle("_active");
                    document.documentElement.classList.toggle("search-form-open");
                } else if (!targetElement.closest(".search-form__body") && document.querySelectorAll(".search-form._active").length > 0) removeClasses(document.querySelectorAll(".search-form._active"), "_active");
                if (targetElement.closest(".catalog__button-more")) {
                    firstItem = document.querySelectorAll(".product-item").length;
                    lastItem = firstItem + 3;
                    initProduct(filteredData, firstItem, lastItem);
                    e.preventDefault();
                }
            };
            ...
            const catalogFilter = document.querySelector(".filter-catalog__items");
            const catalogBody = document.querySelector(".catalog__products");
            const productCardPage = document.querySelector(".page-product-card");
            let filteredData;
            let data;
            let firstItem = 0;
            let lastItem = 6;
            const filterCatalog = (data, filterItems) => {
                const allCategories = [];
                const allBrands = [];
                const ItemLabeles = document.querySelectorAll(`[data-catalog-label='others']  span`);
                const ItemBrands = document.querySelectorAll(`[data-catalog-label='brands']  span`);
                ...
                let sortedBrandsData = {
                    items: []
                };
                filteredData = {
                    items: []
                };
                ItemLabeles.forEach(((item, index) => {
                    allCategories.push(item.innerHTML);
                }));
                ItemBrands.forEach(((item, index) => {
                    allBrands.push(item.innerHTML);
                }));
                data.items.forEach((item => {
                    if (parseInt(filterItems.priceRange.min) * 1e3 <= parseInt(item.price) && parseInt(filterItems.priceRange.max) * 1e3 >= parseInt(item.price)) sortedPriceData.items.push(item);
                }));
                console.log(filteredCategories.brands);
                console.log(filteredCategories.categories);
                if (!filterItems.brands.length) filterItems.brands = allBrands;
                sortedPriceData.items.forEach((item => {
                    if (filterItems.brands) if (filterItems.brands.includes(item.brand)) sortedBrandsData.items.push(item);
                }));
                console.log(sortedBrandsData);
                if (!filterItems.categories.length) filterItems.categories = allCategories;
                sortedBrandsData.items.forEach((item => {
                    if (filterItems.categories.some((category => item.categories.includes(category)))) if (!filteredData.items.includes(item)) filteredData.items.push(item);
                }));
                console.log(filteredData);
                initProduct(filteredData, firstItem, lastItem);
            };
            const getFilterItems = () => {
                const itemsBrandsCheckboxes = document.querySelectorAll(`[data-catalog-input='brands']`);
                const itemsBrandsLabels = document.querySelectorAll(`[data-catalog-label='brands'] span`);
                const itemsCategoriesCheckboxes = document.querySelectorAll(`[data-catalog-input='others']`);
                const itemsCategoriesLabels = document.querySelectorAll(`[data-catalog-label='others']  span`);
                const minPriceInput = document.querySelector(`[data-input-number='min']`);
                const maxPriceInput = document.querySelector(`[data-input-number='max']`);
                filteredCategories.priceRange.min = minPriceInput.value;
                filteredCategories.priceRange.max = maxPriceInput.value;
                filteredCategories.brands = [];
                filteredCategories.categories = [];
                itemsBrandsCheckboxes.forEach(((item, index) => {
                    if (item.checked) if (!filteredCategories.brands.includes(itemsBrandsLabels[index].innerHTML)) filteredCategories.brands.push(itemsBrandsLabels[index].innerHTML);
                }));
                itemsCategoriesCheckboxes.forEach(((item, index) => {
                    if (item.checked) filteredCategories.categories.push(itemsCategoriesLabels[index].innerHTML);
                }));
                filterCatalog(data, filteredCategories);
            };
            const getIdCheckboxItems = () => {
                const ItemCheckboxes = document.querySelectorAll(".filter-catalog-item__checkbox");
                const ItemLabeles = document.querySelectorAll(".filter-catalog-item__label");
                if (ItemCheckboxes.length && ItemLabeles.length) ItemCheckboxes.forEach(((ItemCheckbox, index) => {
                    ItemCheckboxes[index].setAttribute("id", `filter-item-checkbox[${index}]`);
                    ItemLabeles[index].setAttribute("for", `filter-item-checkbox[${index}]`);
                }));
            };
            getIdCheckboxItems();
            if (catalogBody) loadProductsItems();
            const buidProductItem = item => {
                let itemProductTemplate = "";
                let itemProductTemplateBody = "";
                itemProductTemplate += `<article class="catalog__product product-item">`;
                item.image ? itemProductTemplate += `<a href="${item.url}#${item.id}" class="product-item__image product-item__image-ibg">\n        <img src="${item.image}" alt="${item.item}">\n        </a>` : null;
                itemProductTemplateBody += `  <div class="product-item__body">\n        <a href="${item.url}#${item.id}" class="product-item__item">${item.item}</a>\n        <a href="${item.url}#${item.id}" class="product-item__brand">${item.brand}</a>\n        <div class="product-item__price">${item.price} ₽</div>\n      </div>`;
                itemProductTemplate += itemProductTemplateBody;
                itemProductTemplate += `</article>`;
                catalogBody.insertAdjacentHTML("beforeend", itemProductTemplate);
            };
            const initProduct = (data, firstItem, lastItem) => {
                const showMoreButton = document.querySelector(".catalog__button-more");
                const totalItems = data.items.length;
                showMoreButton.hidden = true;
                let dataPart = data.items.slice(firstItem, lastItem);
                dataPart.forEach((item => {
                    buidProductItem(item);
                }));
                if (!dataPart.length) catalogBody.innerHTML = `<div class="filter-catalog-item__no-result">Нет резульатов</div>`;
                if (document.querySelectorAll(".product-item").length >= totalItems) showMoreButton.hidden = true; else showMoreButton.hidden = false;
            };
            ...
            async function loadProductsCardItems() {
                const response = await fetch("files/products.json", {
                    method: "GET"
                });
                if (response.ok) {
            ...
            if (catalogFilter) catalogFilter.addEventListener("change", getFilterItems);
            if (productCardPage) loadProductsCardItems();
            const getProductCard = data => {
                let productCardPageHtml = String(window.location.href);
                if (productCardPageHtml.indexOf("#") !== -1) {
                    let productCardId = productCardPageHtml.slice(productCardPageHtml.indexOf("#") + 1);
                    let productCardItem = data.items.find((item => item.id === productCardId));
                    buildProductCard(productCardItem);
                }
            };
            const buildProductCard = item => {
                console.log(item);
                const productCard = document.querySelector(".product-card__container");
                productCard.innerHTML = "";
                let productCradTemplate = `\n        <div data-da=".product-card__body,991.98,2" class="product-card__images">\n\t\t\t\t\t\t\t<div class="product-card__slider slider-product-card swiper">\n\t\t\t\t\t\t\t\t<div class="slider-product-card__body swiper-wrapper">\n\t\t\t\t\t\t\t\t\t<div class="slider-product-card__slide swiper-slide slider-product-card__slide-ibg">\n\t\t\t\t\t\t\t\t\t\t\n\t\t\t\t\t\t\t\t\t   <img src="${item.image}" alt="${item.item} ${item.brand}">\n\t\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t\t\t\n\t\t\t\t\t\t\t\t\t<div class="slider-product-card__slide swiper-slide slider-product-card__slide-ibg">\n\t\t\t\t\t\t\t\t\t\t\n                  <img src="${item.image}" alt="${item.item} ${item.brand}">\n\t\t\t\t\t\t\t\t\t </div>\n\t\t\t\t\t\t\t\t\t <div class="slider-product-card__slide swiper-slide slider-product-card__slide-ibg">\n\t\t\t\t\t\t\t\t\t\t\n\t\t\t\t\t\t\t\t\t <img src="${item.image}" alt="${item.item} ${item.brand}">\n\t\t\t\t\t\t\t\t\t </div>\n\t\t\t\t\t\t\t\t\t  <div class="slider-product-card__slide swiper-slide slider-product-card__slide-ibg">\n\t\t\t\t\t\t\t\t\t\t\n                    <img src="${item.image}" alt="${item.item} ${item.brand}">\n\t\t\t\t\t\t\t\t\t </div>\n\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t   </div>\n\t\t\t\t\t\t   <div class="product-card__thumb-slider  thumb-slider swiper">\n\t\t\t\t\t\t\t    <div class="thumb-slider__body swiper-wrapper">\n\t\t\t\t\t\t\t\t\t <div class="thumb-slider__slide swiper-slide thumb-slider__slide-ibg">\n                   <img src="${item.image}" alt="${item.item} ${item.brand}">\n\t\t\t\t\t\t\t\t\t </div>\n\t\t\t\t\t\t\t\t\t <div class="thumb-slider__slide swiper-slide thumb-slider__slide-ibg">\n                   <img src="${item.image}" alt="${item.item} ${item.brand}">\n\t\t\t\t\t\t\t\t\t </div>\n\t\t\t\t\t\t\t\t\t <div class="thumb-slider__slide swiper-slide thumb-slider__slide-ibg">\n                   <img src="${item.image}" alt="${item.item} ${item.brand}">\n\t\t\t\t\t\t\t\t\t </div>\n\t\t\t\t\t\t\t\t\t <div class="thumb-slider__slide swiper-slide thumb-slider__slide-ibg">\n                   <img src="${item.image}" alt="${item.item} ${item.brand}">\n\t\t\t\t\t\t\t\t\t </div>\n\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t   </div>\n\t\t\t\t\t\t</div>\n\t\t\t\t\t\t<div class="product-card__content ">\n\t\t\t\t\t\t\t<div class="product-card__body">\n\t\t\t\t\t\t\t\t<h2 class="product-card__title">${item.item} ${item.brand}</h2>\n\t\t\t\t\t\t\t\t<div class="product-card__table table-product-card">\n\t\t\t\t\t\t\t\t\t<div class="table-product-card__categry">Категория: <span>${item.categories[0]}</span></div>\n\t\t\t\t\t\t\t\t\t <div class="table-product-card__brand">Бренд: <span>${item.brand}</span></div>\n\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t\t<div class="product-card__description">\n\t\t\t\t\t\t\t\t\t <p>\n                    ${item.text} \n\t\t\t\t\t\t\t\t\t\t</p>\n\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t\t<div class="product-card__price"> <span>${item.price.replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, "$1 ")} ₽ </span> </div>\n\t\t\t\t\t\t\t\t<div class="product-card__actions actions-product-card">\n\t\t\t\t\t\t\t\t\t <button type="button" class="actions-product-card__button button button--grey">купить</button>\n\t\t\t\t\t\t\t\t\t <button type="button" class="actions-product-card__button button button--border_light ">добавить в корзину</button>\n\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t   \n\t\t\t\t\t\t</div>\n        `;
                productCard.innerHTML = productCradTemplate;
                initSliders();
                da.init();
            };
            const deleteWrongSilder = () => {
                const productCardsliders = document.querySelectorAll(".product-card__images");
                if (productCardsliders.length > 1) {
                    console.log("i have deleted the wrong slider");
                    productCardsliders[1].remove();
                } else return;
            };
            if (productCardPage) window.addEventListener("resize", deleteWrongSilder);
